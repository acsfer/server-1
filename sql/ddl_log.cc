...
#define DDL_LOG_UNIQUE_ID_POS 8
/*
...
*/
#define DDL_LOG_TMP_NAME_POS 16

/* Definitions for the ddl log header */
...
{
  strxmov(file_name, mysql_data_home, FN_DIRSEP, "ddl_log.log", NullS);
}
...
static bool write_ddl_log_file_entry(uint entry_pos)
{
  ...
  DBUG_RETURN(mysql_file_pwrite(file_id, file_entry_buf,
                                IO_SIZE, global_ddl_log.io_size * entry_pos,
                                MYF(MY_WME | MY_NABP)));
  DBUG_RETURN(error);
}
...
static uchar *store_string(uchar *pos, uchar *end, const char *str)
{
  uint32 length;
  if (unlikely(!str))
    str= "";
  ...
  memcpy(pos+2, str, length + 1);               // Copy with end \0
...
static const char *get_string(uchar **pos, const uchar *end, size_t *str_length)
{
  uint32 length;
  if (likely(*pos + 3 < end))
  {
  ...
  DBUG_ASSERT(0);
  *pos= (uchar*) end;                           // Overflow on read
  *str_length= 0;
  return "";
}
...
{
  LEX_CSTRING tmp;
  tmp.str= get_string(pos, end, &tmp.length);
  return tmp;
}
...
static void set_global_from_ddl_log_entry(const DDL_LOG_ENTRY *ddl_log_entry)
{
  ...
  file_entry_buf[DDL_LOG_ENTRY_TYPE_POS]=  (uchar) DDL_LOG_ENTRY_CODE;
  ...
  int8store(file_entry_buf+DDL_LOG_UNIQUE_ID_POS,  ddl_log_entry->xid);
                                          ...
                                          const uint read_entry)
{
  ...
  ddl_log_entry->xid= uint8korr(file_entry_buf + DDL_LOG_UNIQUE_ID_POS);

  pos= file_entry_buf + global_ddl_log.name_pos;
  ddl_log_entry->handler_name= get_string(&pos, end, &length);
  ddl_log_entry->db= get_string(&pos, end);
  ddl_log_entry->name= get_string(&pos, end, &length);
  ddl_log_entry->from_db= get_string(&pos, end);
  ddl_log_entry->from_name= get_string(&pos, end, &length);
  ddl_log_entry->tmp_name= get_string(&pos, end, &length);
}
                                  ...
                                  DDL_LOG_ENTRY *ddl_log_entry)
{
  ...
  handler_name.str=    ddl_log_entry->handler_name;
  ...
  if (!strcmp(ddl_log_entry->handler_name, reg_ext))
    frm_action= TRUE;
  else if (ddl_log_entry->handler_name[0])
  {
  ...
  */
  ...
  case DDL_LOG_RENAME_TABLE_ACTION:
  {
    ...
    */
    ...
    lex_string_set(&from_table, ddl_log_entry->from_name);
    lex_string_set(&to_table, ddl_log_entry->name);
                                          ...
                                          DDL_LOG_MEMORY_ENTRY **active_entry)
{
  ...
  DBUG_ENTER("ddl_log_write_execute_entry_internal");
  ...
  if (!complete)
  {
    ...
    */
    ...
    file_entry_buf[DDL_LOG_ENTRY_TYPE_POS]= (uchar)DDL_IGNORE_LOG_ENTRY_CODE;
  file_entry_buf[DDL_LOG_ACTION_TYPE_POS]= 0; /* Ignored for execute entries */
  file_entry_buf[DDL_LOG_PHASE_POS]= 0;
  int4store(file_entry_buf + DDL_LOG_NEXT_ENTRY_POS, first_entry);
  int8store(file_entry_buf + DDL_LOG_UNIQUE_ID_POS, 0);
...
bool ddl_log_close_binlogged_events(HASH *xids)
{
  ...
  mysql_mutex_lock(&LOCK_gdl);
  for (i= 1; i < global_ddl_log.num_entries + 1; i++)
  {
                          ...
                          const LEX_CSTRING *new_alias)
{
  ...
  ddl_log_entry.handler_name= ha_resolve_storage_engine_name(hton);
  ddl_log_entry.db=           *const_cast<LEX_CSTRING*>(new_db);
  ddl_log_entry.name=         new_alias->str;
  ddl_log_entry.from_db=      *const_cast<LEX_CSTRING*>(org_db);
  ddl_log_entry.from_name=    org_alias->str;
